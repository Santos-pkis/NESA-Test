  ...
  return (
    <div className="p-10 bg-white rounded-lg sm:min-w-[30rem] sm:max-w-[45rem] w-4/5">
      <motion.div {...opacityTrans} className="space-y-4 text-center overflow-y-auto" key={"step-3"}>
        ...
        <p className="font-bold md:text-3xl text-2xl">Thank You for Nominating</p>
        <p>
        ...
        <div className="flex justify-center items-center gap-3 text-xs">
          <div>
            <Link href={"/categories"} onClick={hideModal}>
              <button className="px-5 py-3 rounded-lg text-black bg-deepGold">Nominate in another Category</button>
            </Link>
          </div>
          <button className="px-5 py-3 rounded-lg text-black border-deepGold border">Invite Others to Nominate</button>
        </div>
      </motion.div>
    </div>
  );
